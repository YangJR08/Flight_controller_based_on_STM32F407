...
## 飞控任务
### MPU6050介绍
使用IIC通讯协议，没有必要使用中断引脚，是失重状态会产生中断
使用硬件IIC，IIC1_SDA-PB9,IIC1_SCL-PB8
### MPU6050初始化
1、初始化IIC接口，HAL库自动做了
2、复位MPU6050
让所有寄存器恢复默认值
通过对“电源管理寄存器 1”（0X6B）的bit7写1实现，上电复位后建议延时一段时间。复位后，“电源管理寄存器 1”恢复默认值（0X40），然后必须设置该寄存器为0X00，以唤醒MPU6050，进入正常工作状态。
配置设置陀螺仪和加速度计的量程，采样率等参数，这些参数可以根据需要进行调整，量程范围不是越大与大越好，因为adc的分辨率是固定的，量程范围越大，分辨率就越低，所以需要根据实际需求来设置量程范围，所以量程在够用的范围内越小越好，这样可以提高分辨率，减少噪声的影响。
角速度量程可以选大点+-2000°/s，一圈360°，加速度2g
3、配置低通滤波
无人机飞起来会有高频振动，但是这对于我们来说并不要高频变化的信号，所以需要低通滤波。
加速度配置184陀螺仪配置188，DLPF_CFG值为1
4、陀螺仪测量值 寄存器地址67-72，就是存了三个陀螺仪adc数据，高位在前
5、初始化后获取零偏值后续采集的值减去零偏值
### 开始飞控任务
1、解决数据抖动，滤波问题
#### 卡尔曼滤波
抖动的越厉害滤波越准，抖动越小滤波越小
均值和期望-求平均值
方差和标准差-一组数据在平均值附近波动幅度
正态分布-
随机变量-描述系统隐藏状态，测量出来的结果以均值和结果方差
估计准度和精度-准度误差要够小，计算输出值和真值越接近，精度-计算输出值同级方差低，波动小
#### 通过加速度和角速度计算姿态
1、角速度积分，可以得到相应角度，积分三个轴就知道空间中姿态
优点简单，稳定，运算简单，缺点长期会出现积分误差，欧拉角存在万向锁
2、加速度计算，受力分析
飞机发生倾斜之后，重力加速度不再垂直于芯片，而发生了倾斜，可以这样计算角度
优点简单，缺点容易受到抖动影响，不能计算偏航角的角度
3，互补解算
结合上面两者，结合上面两者优点，缺点，受振动影响，动态性能差，大角度变化反应慢
4、四元数姿态解算
#### 三轴pid控制
俯仰角、横滚角、偏航角，每个都是串级pid，分内外环
知道目标是如何输入来的，需要平稳飞行角度为零，需要向前飞行就是遥控器向前
外环的输出值是内环的目标值
外环的测量值通过姿态解算得到俯仰角，内环是角速度本来就有，mpu6050直接得到的
内环输出值控制电机的转速，控制pwm占空比，电机的分组配置，这样才能前后飞行
pid建模先建模外环再建模内环
##### 俯仰角
调参先调内环再调外环，先调内环P
1、内环参数P合理，实现的效果会阻碍无人机下落有明显的停顿，P过大会超出限幅，所以不行
2、再调节外环的P，外环合理，就能平稳飞行到架子上
+1，不能保持稳定，-1能有一点稳定效果，判断极性
然后判断大小-5，还不错
3、再调内环微分部分，极性和比例与外环部分保持一致
调节大小，（err-lasterr）/🔺t（0.006）
#### 横滚角
和俯仰角调参步骤一样，参数一样，主要是确定极性
#### 偏航角
偏航角pid参数和俯仰角横滚角不同，需要重新调节pid
依旧先调节内环判断极性，调对有明显助力
再调外环，外环调对有明显回正
内环外环参数过大，都是作用到4个电机上，会影响俯仰角和横滚角的调节
偏航角属于辅助功能，保证别在空中旋转
### 定高功能实现
引脚配置
IIC2,涉及I2C2_SDA-PF0,I2C2_SCL-PF1，VX_XSHUT-PB12,VX_GPIO1-PB13
st公司的芯片
移植驱动
实现iic通讯
1、芯片重置先拉低再拉高XSHUT，延时2ms
2、读字节判断16位数据高八位在前还是低八位在前
3、读取状态寄存器等待重置完成
4、配置寄存器
初始化方法
设置一下远近距离模式
...
6、获取测距值
#### 大端序和小端序
stm32默认使用小端序，16位数据低八位在前高八位在后
这个激光测距仪它的端序是高八位在前，低八位在后
不论读写操作都要转换成大端序
### 定高pid
单环还是串级pid的选择
串级pid的优势，控制更加及时，缺点结构更加复杂，需要测量值更多，保证外环输出内环输入单位能匹配上，调参复杂
单环，相应速度不够灵敏，响应没有这么快
#### 记录目标值
进入定高状态一瞬间，记录当前高度为pid目标值
